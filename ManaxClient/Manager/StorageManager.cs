...
public static class StorageManager
{
    ...
    public static void ClearTrash()
    {
        Task.Run(() =>
        {
            try
            {
                foreach (string file in Directory.GetFiles(TrashFolder)) File.Delete(file);
                foreach (string directory in Directory.GetDirectories(TrashFolder)) Directory.Delete(directory, true);
            }
            catch (Exception e)
            {
