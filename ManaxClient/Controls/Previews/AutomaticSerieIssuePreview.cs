...
public class AutomaticSerieIssuePreview : Button
{
    public static readonly AttachedProperty<AutomaticIssueSerieDto> IssueProperty =
        AvaloniaProperty.RegisterAttached<AutomaticSerieIssuePreview, Grid, AutomaticIssueSerieDto>(
            "Issue", new AutomaticIssueSerieDto(), false, BindingMode.OneTime);
    ...
    public AutomaticSerieIssuePreview()
    {
        ...
        issueTitle.Bind(TextBlock.TextProperty, new Binding("Issue.Problem")
        {
            ...
            Converter = new FuncValueConverter<AutomaticIssueSerieType, string>(problem =>
                problem switch
                {
                    ...
                    AutomaticIssueSerieType.PosterMissing => "Affiche manquante",
                    AutomaticIssueSerieType.PosterTooSmall => "Affiche trop petite",
                    AutomaticIssueSerieType.MissingChapter => "Chapitre manquant",
                    AutomaticIssueSerieType.DescriptionTooShort => "Description trop courte",
                    AutomaticIssueSerieType.DescriptionTooLong => "Description trop longue",
                    _ => problem.ToString()
                })
        });
        ...
        issueDetails.Bind(TextBlock.TextProperty, new Binding("Issue")
        {
            Source = this,
            Mode = BindingMode.OneWay,
            Converter = new FuncValueConverter<AutomaticIssueSerieDto, string>(issue =>
                issue != null ? $"Série ID: {issue.SerieId} • {issue.CreatedAt:dd/MM/yyyy HH:mm}" : string.Empty)
        });
    ...
    public static void SetIssue(AvaloniaObject element, AutomaticIssueSerieDto issueValue)
    {
        element.SetValue(IssueProperty, issueValue);
    }

    public static AutomaticIssueSerieDto GetIssue(AvaloniaObject element)
    {
