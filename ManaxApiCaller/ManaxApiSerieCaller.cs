...
public static class ManaxApiSerieCaller
{
    public static async Task<List<long>?> GetSeriesIdsAsync()
    {
        HttpResponseMessage response = await ManaxApiCaller.Client.GetAsync("api/series");
    ...
    {
        HttpResponseMessage response = await ManaxApiCaller.Client.GetAsync($"api/serie/{id}");
        if (!response.IsSuccessStatusCode) return null;
    ...
    {
        HttpResponseMessage response = await ManaxApiCaller.Client.GetAsync($"api/series/{id}/chapters");
        if (!response.IsSuccessStatusCode) return null;
        return await response.Content.ReadFromJsonAsync<List<long>>();
    }
    ...
    {
        HttpResponseMessage response = await ManaxApiCaller.Client.PostAsJsonAsync("api/serie", serie);
        if (!response.IsSuccessStatusCode) return null;
    ...
    {
        HttpResponseMessage response = await ManaxApiCaller.Client.PutAsJsonAsync($"api/serie/{id}", serie);
        return response.IsSuccessStatusCode;
    }

    public static async Task<bool> DeleteSerieAsync(long id)
    {
        HttpResponseMessage response = await ManaxApiCaller.Client.DeleteAsync($"api/serie/{id}");
        return response.IsSuccessStatusCode;
    }
}
